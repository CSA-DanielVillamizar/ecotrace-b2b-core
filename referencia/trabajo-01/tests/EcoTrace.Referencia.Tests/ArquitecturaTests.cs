...
public sealed class ArquitecturaTests
{
    ...
    private static string RaizDeSolucion()
    {
        ...
        while (directorio is not null && !File.Exists(Path.Combine(directorio.FullName, "EcoTrace.Referencia.sln")))
        {
            directorio = directorio.Parent;
        }

        return directorio?.FullName ?? throw new InvalidOperationException("No se encontro EcoTrace.Referencia.sln.");
    }
}
